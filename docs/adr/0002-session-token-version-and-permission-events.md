...
2. **事件**（仍为 notify-only，REST 为真相源）：
   - `permissions.updated` — 用户角色、角色权限变更或角色删除后定向推送；客户端重新 `GET /users/me`，**不**强制登出。
...
5. **部署级 Redis 频道**：`REALTIME_REDIS_CHANNEL`（默认 `toolhub:realtime`，可用 `TOOLHUB_REALTIME_REDIS_CHANNEL`）命名空间频道。多套部署共用同一 Redis 时必须配置不同频道，避免 `user_id` 重叠导致 `session.revoked` / `permissions.updated` 串台（仅选不同 Redis DB 不够，Pub/Sub 不按 DB 隔离）。
...
| 固定全局 Redis 频道 | 拒绝：多部署共用 Redis 会串台 |
...
- 多实例：同一部署的所有实例共享同一 `REDIS_URL`、`REALTIME_REDIS_CHANNEL` 与 `SECRET_KEY`；envelope 带 `origin` 跳过本机回环。
