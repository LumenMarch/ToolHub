...
- Both embed the frontend build (the repo's `frontend/dist` is embedded at `<bundle>/frontend`; see §7.6 for the runtime mount mechanism).
...
echo "" | uv run scripts/build-bundle.py # piped stdin (CI): same as -y
```

The menu is a **keyboard checklist** (no numbered input; rendered with rich — a `Panel` titled "ToolHub Bundle Builder", redrawn in place via `rich.live.Live`; no curses, since Windows cmd has no curses): `↑`/`↓` move the highlight, `Space` toggles an item, `Enter` runs **all checked items in menu order**, `q` or Ctrl-C quits. `Build standalone` and `Exit` are checked by default (pressing Enter builds standalone and exits in one step); `standalone` and `onefile` are mutually exclusive (toggling one automatically deselects the other). After the checked actions finish, the menu is redrawn with the selection reset to the default (`standalone` + `exit`). Items: `Build standalone (default)`, `Build onefile`, `Clean build artifacts`, `Show help`, `Exit` (pressing Enter runs checked items in menu order, so `standalone` builds then `Exit` quits; `Show help` prints this help and returns to the menu). On **arm64 hosts** the menu additionally shows an architecture row above the items — `Arch: arm64  (Space toggles arm64/x86_64)` (initial value: host arch, or the `--arch` value when given). The cursor can land on that row and `Space` toggles the build target; it is **not an action** (never checked, `Enter` never runs it), and the architecture persists across menu redraws while the checked selection resets. Key reading is cross-platform stdlib: `termios` cbreak + byte reads on POSIX (arrow keys `ESC [ A`/`ESC [ B`), `msvcrt.getwch()` on Windows (arrows arrive as a `\x00`/`\xe0` prefix + scan code, two calls in `_read_key()`).
...
**Architecture selection (`--arch arm64|x86_64`, added 2026-08-03)**: by default the bundle targets the host architecture and the behavior is unchanged. `--arch` switches **only the Python interpreter architecture** — uv fetches a CPython 3.13 build for the target platform (`uv venv --python cpython-3.13.14-<os>-<arch>-<libc>`, uv 0.12.1 naming) and Nuitka follows the interpreter; no compiler flags, no universal binaries. Cross builds are supported **only on arm64 hosts** (macOS arm64 / Windows ARM64 / Linux arm64); on any other host `--arch` is a usage error (exit 2) and the menu hides the arch row. Running an x86_64 interpreter on an arm64 host needs a compatibility layer — **Rosetta 2 on macOS** (`softwareupdate --install-rosetta`), the **built-in x64 emulation on Windows ARM64**, **qemu-user + binfmt_misc on Linux arm64** — and the script verifies the created venv's interpreter by executing it: if it cannot run (layer missing) or reports the wrong architecture, the build fails with exit 1 and platform-specific guidance. Cross-arch venvs live in `build/venv-<arch>` (host builds keep `build/venv`), so both can coexist; `clean` removes the whole `build/`.
...
The frontend is embedded with `--include-data-dir=<frontend>/dist=frontend`, so the built files land at `<bundle>/frontend/...` next to the binary. `backend/app/main.py` (comments in Chinese) resolves the dist dir in priority order:

1. **Repo/dev layout**: `Path(__file__).resolve().parents[2] / "frontend" / "dist"` — used when running from source.
2. **Nuitka frozen layout**: `Path(__file__).resolve().parents[1] / "frontend"` — used when the dev path does not exist. (After compilation `app/main.py`'s `__file__` is the virtual path `<bundle>/app/main.py`, so `parents[1]` is the bundle dir — verified experimentally.)
