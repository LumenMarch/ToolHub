...
def run_comparisons(req: ComparisonRequest):
    ...
    if req.thisFinance and req.lastFinance:
        try:
            ...
            ff.Finance_check()
            diff_count = _safe_len(ff.check_Custodian) + _safe_len(ff.check_Department) + _safe_len(ff.new_Custodian_assets) + _safe_len(ff.removed_Custodian_assets) + _safe_len(ff.new_Department_assets) + _safe_len(ff.removed_Department_assets)
            results_info.append({
                "key": "ff",
                "label": "【财务-财务】",
            ...
            results_info.append({"key": "ff", "label": "【财务-财务】", "has_diff": False, "msg": f"异常: {e}"})
    summary['ff'] = ff
    ...
    if req.thisSFC and req.lastSFC:
        try:
            ...
            results_info.append({"key": "sfc", "label": "【SFC-SFC】", "has_diff": False, "msg": f"异常: {e}"})
    summary['sfc'] = sfc
    ...
    if req.thisNotes and req.lastNotes:
        try:
            ...
            diff_count = _safe_len(nn.new_assets) + _safe_len(nn.removed_assets) + abs(_safe_len(nn.new_No_assets) - _safe_len(nn.removed_No_assets))
            results_info.append({
                "key": "nn",
                "label": "【Notes-Notes】",
                "has_diff": diff_count > 0,
            ...
            results_info.append({"key": "nn", "label": "【Notes-Notes】", "has_diff": False, "msg": f"异常: {e}"})
    summary['nn'] = nn
    ...
    if req.thisCustomer and req.lastCustomer and req.driData:
        try:
            ...
            diff_count = _safe_len(cc.new_Customer_assets) + _safe_len(cc.removed_Customer_assets)
            results_info.append({
                "key": "cc",
                "label": "【客户-客户】",
                "has_diff": diff_count > 0,
            ...
            results_info.append({"key": "cc", "label": "【客户-客户】", "has_diff": False, "msg": f"异常: {e}"})
    summary['cc'] = cc
    ...
    if req.thisFinance and req.thisNotes and req.custodianData:
        try:
            ...
            results_info.append({"key": "fn", "label": "【财务比Notes】", "has_diff": False, "msg": f"异常: {e}"})
    summary['fn'] = fn
    ...
    if req.thisNotes and req.thisSFC:
        try:
            ...
            diff_count = _safe_len(ns.Notes_new_assets) + _safe_len(ns.Notes_removed_assets)
            results_info.append({
                "key": "ns",
                "label": "【Notes比SFC】",
                "has_diff": diff_count > 0,
            ...
            results_info.append({"key": "ns", "label": "【Notes比SFC】", "has_diff": False, "msg": f"异常: {e}"})
    summary['ns'] = ns
    ...
    if req.thisCustomer and req.thisNotes and req.driData:
        try:
            ...
            results_info.append({"key": "cn", "label": "【客户比Notes】", "has_diff": False, "msg": f"异常: {e}"})
    summary['cn'] = cn
    ...
    for key, val in req_reviews.items():
        if key in combo_to_cell:
            ...
            for coord in coords:
                cell = ws[coord]
                # 若为0填充绿色，反之填选项色
                cv = str(cell.value).strip().replace("/", "") if cell.value is not None else ""
    ...
    try:
        ...
        if fs_res or ff:
            ...
            if getattr(ff, "new_Custodian_assets", []):
                ...
                ff_dict["依保管人新增"] = df[df["資產編號"].isin(list(ff.new_Custodian_assets))][["資產名稱", "資產編號", "保管人員"]].drop_duplicates(subset=["資產編號"])
            if getattr(ff, "removed_Custodian_assets", []):
                df = _safe_to_pandas(ff.last_Finance_data)
                ff_dict["依保管人减少"] = df[df["資產編號"].isin(list(ff.removed_Custodian_assets))][["資產名稱", "資產編號", "保管人員"]].drop_duplicates(subset=["資產編號"])
            if getattr(ff, "new_Department_assets", []):
                df = _safe_to_pandas(ff.this_Finance_data)
                ff_dict["依部门新增"] = df[df["資產編號"].isin(list(ff.new_Department_assets))][["資產名稱", "資產編號", "資產所屬部門代號"]].drop_duplicates(subset=["資產編號"])
        ...
        if ns_res or nn:
            ...
            if getattr(nn, "new_assets", []):
                ...
                nn_dict["本月新增"] = df[df["資產編號"].isin(list(nn.new_assets))][["資產名稱", "資產編號", "保管人"]].drop_duplicates()
        ...
        if sfc_res or sfc:
            ...
            if getattr(sfc, "new_assets", []):
                ...
                ss_dict["本月新增"] = df[df["资产编号"].isin(list(sfc.new_assets))][["设备名称", "资产编号", "保管人"]].drop_duplicates()
        ...
        if cs_res or cc:
            ...
            if getattr(cc, "new_Customer_assets", []):
                ...
                cc_dict["本月新增"] = df[df["Asset ID"].isin(list(cc.new_Customer_assets))][["DRI", "Asset ID", "RFID"]].drop_duplicates()
        ...
        if fns_res or fn:
            ...
            if getattr(fn, "new_assets", []):
                ...
                fn_dict["Notes比财务新增资产"] = df[df["資產編號"].isin(list(fn.new_assets))][["資產名稱", "資產編號", "保管人"]].drop_duplicates()
        ...
        if nss_res or ns:
            ...
            if getattr(ns, "Notes_new_assets", []):
                ...
                ns_dict["Notes有且SFC无"] = df[df["資產編號"].isin(list(ns.Notes_new_assets))][["資產名稱", "資產編號", "保管人"]].drop_duplicates()
        ...
        if cns_res or cn:
            ...
            if getattr(cn, "remove_assets", []):
                ...
                cn_dict["客户有且Notes无"] = df[df["RFID"].isin(list(cn.remove_assets))][["DRI", "Asset ID", "RFID"]].drop_duplicates()
            if getattr(cn, "new_assets", []):
                df = _safe_to_pandas(cn.this_Notes_data)
                cn_dict["Notes有且客户无"] = df[df["RFID（Tag）"].isin(list(cn.new_assets))]
            if cn_dict:
                comparisons.append(("7-Notes客户资产 VS 客户系统资产", cn_dict, req.remarks.get("cn", "")))
