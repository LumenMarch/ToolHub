"""atlas-merge 结果短时缓存（仿 attendance 的 AttendanceResultCache）。

以统一任务产物存储（TaskArtifactStore）为后端，进程内维护
result_id → 元数据索引，TTL 600s，支持 put/get/delete 与归属校验。
"""
...
TASK_TOOL = "atlas-merge"
RESULT_PATH = "artifacts/result.csv"


class AtlasMergeResultNotFoundError(LookupError):
    """请求的合并结果不存在或不属于当前用户。"""


class AtlasMergeResultExpiredError(LookupError):
    """请求的合并结果已过期。"""


@dataclass(frozen=True)
class CachedAtlasMergeResult:
    result_id: str
    user_id: int
    filename: str
    content_path: Path
...
class AtlasMergeResultCache:
    """以统一任务产物存储为后端的合并结果短时缓存。"""
    ...
    def __init__(
        ...
        *,
        ...
        self._entries: OrderedDict[str, CachedAtlasMergeResult] = OrderedDict()
    ...
    def get(self, result_id: str, user_id: int) -> CachedAtlasMergeResult:
        now_monotonic = time.monotonic()
        with self._lock:
    ...
    def _load_entry(
        ...
        try:
            ...
            if unavailable_user_id == user_id:
                raise AtlasMergeResultExpiredError from exc
            raise AtlasMergeResultNotFoundError from exc
...
atlas_merge_result_cache = AtlasMergeResultCache()
